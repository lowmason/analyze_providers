...
def fetch_ces(bls: 'BLSClient', start_year: int = 2019, end_year: int = 2026, force_refresh: bool = False) -> pl.DataFrame:
    ...
    Output columns: ref_date (date with day=12), supersector, employment, seasonal_adjustment.
    ...
    Returns:
        Eager DataFrame with ref_date, supersector, employment, seasonal_adjustment.
    """
    ...
    # Map series_id to supersector and seasonal; then pivot or reshape to ref_date, supersector, employment, seasonal_adjustment
    # eco-stats returns: series_id, date, year, period, period_name, value. date has day=12.
    if 'date' in raw.columns:
        ref_date = pl.col('date')
    else:
        ref_date = pl.struct(['year', 'period']).alias('ref_date')

    # Build supersector and seasonal from series_id (e.g. CES0000000001 -> 00, SA)
    ...
    out = (
        ...
        .select(['ref_date', 'supersector_code', 'employment', 'seasonal_adjustment'])
        .with_columns(pl.col('supersector_code').alias('supersector'))
    )
